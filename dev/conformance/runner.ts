...
import {ApiMapValue, DocumentData} from '../src/types';
...
function runTest(spec: ConformanceProto) {
  ...
  const updateTest = (spec: ConformanceProto) => {
    const overrides: ApiOverride = {commit: commitHandler(spec)};
    return createInstance(overrides).then(() => {
      ...
      // TODO(mrschmidt): Remove 'any' and call by calling update directly for
  ...
  if (getSpec) {
    ...
    return Promise.reject(new Error(`Unhandled Spec:
    ${JSON.stringify(spec)}`));
  }
